...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.
                ...
                .and()
                    .authorizeRequests() // URL별 권한 관리를 설정하는 옵션, authorizeRequests()가 선언되어야 antMatchers 옵션을 사용할 수 있다.
                    .antMatchers("/","/css/**","/images/**","/js/**", "/h2-console/**", "/profile").permitAll()
                    .antMatchers("/api/v1/**").hasRole((Role.USER.name()))
                    .anyRequest().authenticated()// 설정된 값들 이외 나머지 URL들을 나타낸다. 나머지 URL들은 모두 인증된 사용자들에게만 허용하게 한다, 인증된 사용자 = 로그인한 사용자
                .and()
                    .logout()
                        .logoutSuccessUrl("/")
                .and()
